...
if __name__ == '__main__':
    ...
    # print(val.head())
    root ='/content/drive/Shareddrives/huent/thesis/dataset/etl/etl8g'
    df = pd.read_csv(root + '/train.csv')
    print(df.head())
    train, test = sklearn.model_selection.train_test_split(df, test_size=0.9, random_state=42, shuffle=True, stratify=df['label_idx'])
    train.to_csv(os.path.join(root, 'train_small.csv'), index=False)
    df = pd.read_csv(root + '/val.csv')
    print(df.head())
    train, test = sklearn.model_selection.train_test_split(df, test_size=0.9, random_state=42, shuffle=True, stratify=df['label_idx'])
    train.to_csv(os.path.join(root, 'val_small.csv'), index=False)